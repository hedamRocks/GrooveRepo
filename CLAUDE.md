...
- ✅ **Post-facelift audit fixes** (perf / flow / UX)
  ...
  - **DJ analysis moved to a standalone worker** (serverless can't run yt-dlp/ffmpeg or long jobs). `/api/analysis/start` now only **enqueues** a pending `AnalysisJob`; a separate process (`worker/index.ts`, run via `npm run worker` or Docker) polls Neon, claims jobs with `FOR UPDATE SKIP LOCKED`, and runs the existing pipeline, writing results back to the shared DB. Required making the analysis utils standalone (explicit `prisma` imports in analysis-worker/bpm-normalizer/energy-calculator; removed the in-memory queue; un-hardcoded the yt-dlp cookies path). Run locally (Mac, free) or on a host — see `worker/README.md` and `docs/worker-deployment.md` (Railway/Fly/Hetzner). Added `tsx` devDep + `Dockerfile`/`docker-compose.yml`.
  - **DJ analysis reliability overhaul** (`server/utils/audio-analyzer.ts` + worker):
    - **Key detection rewritten** — replaced single-pitch autocorrelation chroma with a real FFT (`server/utils/fft.ts`, 16384-pt) magnitude→pitch-class chroma, and **fixed a rotation-direction bug** in the Krumhansl matching that had been producing tritone-wrong keys. Adds confidence (margin over runner-up) + Camelot notation. Verified against synthetic chords (C maj→8B, A min→8A, A maj→11B).
    - **Multi-window BPM consensus** — analyzes 3 windows across a 60s sample (was a single 30s clip) and takes the agreement; confidence reflects agreement, octave choice still deferred to the genre-aware normalizer.
    - **Real loudness** — energy now uses ffmpeg EBU R128 (`loudnorm`) integrated LUFS instead of ad-hoc RMS.
    - **Assisted-verify workflow** — `Track.keyConfidence` + `Track.needsReview` (low-confidence flag); setlist rows show a "verify" badge, the detected key, and one-tap **÷2 / ×2** octave correction. (Pipeline runs locally only — needs ffmpeg/yt-dlp + YOUTUBE_API_KEY.)
